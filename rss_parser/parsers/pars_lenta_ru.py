...
class LentaRuParser(AbstractNewsParser):
    def __init__(self, link: str):
        super().__init__(link)
        if 'lenta.ru' not in self.domain:
            raise ValueError(f"Ссылка '{link}' не принадлежит домену lenta.ru")
    ...
    def _parse_html_sync(html_content: str) -> str|None:
        ...
        paragraphs = text_div.find_all('p',  class_="topic-body__content-text")
        for p in paragraphs:
...
async def main():
    lenta_ru_link = "https://lenta.ru/news/2025/10/04/13-letniy-pyanyy-malchik-ugnal-avtomobil-i-popal-v-avariyu/"
    
    try:
        parser = LentaRuParser(lenta_ru_link)
