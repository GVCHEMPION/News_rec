...
class MailRuParser(AbstractNewsParser):
    def __init__(self, link: str):
        super().__init__(link)
        if 'mail.ru' not in self.domain:
            raise ValueError(f"Ссылка '{link}' не принадлежит домену mail.ru")

    @staticmethod
    def _parse_html_sync(html_content: str) -> str|None:
        soup = BeautifulSoup(html_content, 'lxml')
        
        selector = 'header header p, main[itemprop="articleBody"] p'
...
async def main():
    mail_ru_link = "https://news.mail.ru/politics/68170968/"
    
    try:
        parser = MailRuParser(mail_ru_link)
